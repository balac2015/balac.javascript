...
function getOrder(req, res, callback) {
    ...
    queryOrder(querys, function (err, order) {
        if (!err) {
            callback(err, order);
        }
    });
 ...
 * @param callback 回调函数进行返回结果
 */
function getOrderByFranchisee(req, res, callback) {
...
exports.exportOrder = function (req, res) {
    ...
    if (isFranchisee) {
        getOrderByFranchisee(req, res, function (err, orders) {
            conf.rows = orders;     //数据内容
            var result = excal.execute(conf);
            var filename = "order" + moment().format("YYYYMMDDhhmmss") + ".xlsx";  //生成文件名
