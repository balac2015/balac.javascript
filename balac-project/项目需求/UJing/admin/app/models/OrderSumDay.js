 ...
 * @type {Schema}
 */
var OrderSumDaySchema = new Schema({
...
OrderSumDaySchema.plugin(CreateUpdatedAt)

OrderSumDaySchema.methods = {
    //分页功能
    findTopByAllowance: function (querys,callback) {
        //q:查询条件
        //排序功能默认用插入时间倒叙：sort(‘-create_date’)
        var query = OrderSumDay.find(querys).sort({"allowance":-1}).limit(10);
...
var OrderSumDay = mongoose.model('OrderSumDay', OrderSumDaySchema)
