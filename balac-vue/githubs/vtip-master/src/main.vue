<template>
    <transition name="v-tip-fade">
        <div class="v-tip-container" :class="boxClass" :style="boxStyle" v-show="visible"
            @mouseenter="showTip" @mouseleave="hiddenTip(true)">

            <div class="v-tip-arrows" :class="placement" :style="arrowBox" v-show="placement"></div>
            <span v-if="title" class="v-tip-title">{{ title }}</span>
            <p v-if="content" class="v-tip-content" :style="contentHeight">{{ content }}</p>
            <component v-if="customComponent"
                v-bind="customProps" v-on="customListeners" :is="customComponent"
                @hidden-tip="hiddenTip" @update-tip="updateTip"></component> 
...
<script>
    ...
    import { debounce, checkScrollable, getScrollContainer, computeArrowPos, computePlacementInfo, computeCoordinateBaseMid, computeCoordinateBaseEdge } from './util';
    ...
    export default {
        ...
        props: {
            ...
            customProps: { type: Object, default () { return {} }},
            // 对应 <component> 组件 is 属性
            customComponent: { type: [String, Function, Object], default: '' },
            // 用于监听自定义组件 emit 的事件
            ...
            transition: Boolean,
            // 提示用的小箭头大小
            arrowsSize: { type: Number, default: 8 },
            // 组建的宽度
        ...
        computed: {
            ...
            boxClass () {
                ...
                const tsClass = transition ? 'transition-transform' : '';
